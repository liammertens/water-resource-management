...
env = mo_gym.make('water-reservoir-v0', normalized_action=False, nO=2, penalize=True)
...
agent = GPIPDContinuousAction(env=env, per=True, dyna=False, experiment_name='gpi-ls_2_obj_RestrAction_pen')

agent.train(500000, env, ref_point=np.array([0,0], dtype=np.float32)) #random ref_point
