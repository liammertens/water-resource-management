...
class Model(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int, reward_dim: int, scaling_factor: np.ndarray, hidden_dim: int = 64, continuous_actions = False):
...
class PCN(MOAgent, MOPolicy):
    ...
    def __init__(
        ...
        self.model = Model(
            self.observation_dim, self.action_dim, self.reward_dim, self.scaling_factor, hidden_dim=self.hidden_dim, continuous_actions=self.continuous_actions
        ).to(self.device)
    ...
    def update(self):
        ...
        if self.continuous_actions:
            # use MSE loss to measure difference between predicted actions from model and those from the saved trajectories
            l = F.mse_loss(log_prob, th.tensor(actions))
            l.backward()
            self.opt.step()
        else:
